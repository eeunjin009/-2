...
void loop() { 
  ...
  if ((distance <= _DIST_MIN) || (distance >= _DIST_MAX)) {
    pwm_val = 255; // LED OFF(최소 밝기)
  } else if (distance <= 200.0) {
    // 100~200mm : 밝기 선형 증가, 200mm에서 최대 밝기
    pwm_val = 255 - (int)((distance - _DIST_MIN) * (255.0 / (200.0 - _DIST_MIN)));
  } else {
    // 200~300mm : 밝기 선형 감소, 300mm에서 최소 밝기
    pwm_val = 255 - (int)((_DIST_MAX - distance) * (255.0 / (_DIST_MAX - 200.0)));
  }
